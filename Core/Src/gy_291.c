...
uint8_t ADXL345_Init(I2C_HandleTypeDef *I2Cx)
{
	...
	uint8_t data;
 	HAL_StatusTypeDef sts = HAL_I2C_Mem_Read(I2Cx, ADXL345_ADDR, REG_WHO_AM_I,1,&wai_check,1,i2c_timeout);
	if (wai_check == 104){

		/*data = 0b01000000; //- | FIFO_EN | I2C_MST_EN | I2C_IF_DIS | - | FIFO_RESET | I2C_MST_RESET | SIG_COND_RESET
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_USER_CTRL, 1, &data, 1, i2c_timeout);*/
/*
		data = 0;
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_PWR_MGMT_1, 1, &data, 1, i2c_timeout);

		data = 0;
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_SMPRT_DIV, 1, &data, 1, i2c_timeout);

		data = 0;	//FSYNC DLPF
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_CONFIG, 1, &data, 1, i2c_timeout);

		data = 0; //250 deg/s
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_GYRO_CONFIG, 1, &data, 1, i2c_timeout);

		data = 0b00010000; //8g
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_ACCEL_CONFIG, 1, &data, 1, i2c_timeout);

		data = 0b01111000;
		HAL_I2C_Mem_Write(I2Cx, ADXL345_ADDR, REG_FIFO_EN, 1, &data, 1, i2c_timeout);
		return 0;
	}
	return 1;
}
	...
	void ADXL345_Read_Accel(I2C_HandleTypeDef *I2Cx, ADXL345_accel_vectors_t *DataStruct)
		{
			...
			HAL_I2C_Mem_Read(I2Cx, ADXL345_ADDR, REG_ACCEL_XOUT_H, 1, recive_Bytes, 6, i2c_timeout);
	...
	void ADXL345_Read_Gyro(I2C_HandleTypeDef *I2Cx, ADXL345_gyro_vectors_t *DataStruct)
		{
			uint8_t recive_Bytes[6];
			HAL_I2C_Mem_Read(I2Cx, ADXL345_ADDR, REG_GYRO_XOUT_H, 1, recive_Bytes, 6, i2c_timeout);
